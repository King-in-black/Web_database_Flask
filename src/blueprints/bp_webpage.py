...
warnings.filterwarnings(
    "ignore", message="Trying to unpickle estimator", module="sklearn.base"
)
model_path = os.path.join(
    os.path.dirname(__file__), "..", "ML_models", "random_forest_model.pkl"
)
...
def register():
    ...
    if request.method == "POST":
        ...
        if role == "player":
            ...
            new_user = Player(
                Player_ID=player_id, password=password, Trainer_ID=trainer_id
            )
    ...
    if request.method == "POST":
        ...
        if role == "player":
            ...
            if user:
                ...
                return redirect(
                    url_for("webpage.predict")
                )  # incomplete player page for prediction the result
            else:
            ...
            user = Trainer.query.filter_by(
                Trainer_ID=user_id, password=password
            ).scalar()
            if user:
                # when the trainer login successfully, the following steps will be inplemented
